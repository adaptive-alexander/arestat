...
impl Stats {
    ...
    fn get_stats(&mut self) {
        ...
        // Ranges
        (self.min, self.max) = (
            *self.timers.iter().min().unwrap(),
            *self.timers.iter().max().unwrap(),
        );
        ...
        self.bin_lower_range = (0..10)
            .map(|v| self.p05 + v * self.bin_size)
            .collect::<Vec<_>>();
    }
    pub fn pretty_print(&self) {
        ...
        if self.requests > 10 {
            ...
            for (i, bin) in self.bins.iter().enumerate() {
                let range = if i == self.bins.len() - 1 {
                    format!(
                        "{} - {} {}",
                        self.bin_lower_range[i] / time_scale_factor + 1,
                    ...
                    format!(
                        "{} - {} {}",
                        self.bin_lower_range[i] / time_scale_factor,
                        self.bin_lower_range[i + 1] / time_scale_factor,
                        time_unit
                    )
                } else {
                    format!(
                ...
                println!(
                    ...
                    range,
                    "",
                    (bin.len() as f64 * bar_scale_factor) as usize
                );
            }
        }
        ...
        println!(
            ...
            self.avg_time_per_request / time_scale_factor,
            time_unit
        );
        println!(
            "{:<25}{} {}",
            "Avg latency:",
            self.avg_latency / time_scale_factor,
            time_unit
        );
        println!(
            "{:<25}{} {}",
            "Median latency:",
            self.median_latency / time_scale_factor,
            time_unit
        );
        println!(
            "{:<25}{}, {} {}",
            "min, max:",
            self.min / time_scale_factor,
            self.max / time_scale_factor,
            time_unit
        );
        println!(
            "{:<25}{} - {} {}",
            "IQR",
            self.p25 / time_scale_factor,
            self.p75 / time_scale_factor,
            time_unit
        );
        println!(
            "{:<25}{} {}",
            "P95:",
            self.p95 / time_scale_factor,
            time_unit
        );
        println!(
            "{:<25}{} {}",
            "P99:",
            self.p99 / time_scale_factor,
            time_unit
        );
    }
}
...
fn fmt_time(total_time: u128) -> String {
    ...
    match elapsed {
        ...
        10_001..=10_000_000 => {
            format!("{} ms", elapsed / 1_000)
        }
        10_000_001..=60_000_000 => {
            format!("{}.{} s", elapsed / 1_000_000, (elapsed % 1_000_000) / 1000)
        }
        _ => {
            format!(
                "{}:{}.{} ",
                elapsed / 60_000_000,
                (elapsed % 60_000_000) / 1_000_000,
                (elapsed % 1_000_000) / 1000
            )
        }
    }
}
